...
while ws.cell(row_index,1).value != None:
    ...
    unit_dic.update({"skill_type": ws.cell(row_index, 4).value})
    unit_dic.update({"match_type": ws.cell(row_index, 5).value})
    unit_dic.update({"icon": clean_null(ws.cell(row_index, 6).value)})
    unit_dic.update({"name": clean_null(ws.cell(row_index, 7).value)})

    buff_list = []
    buff_dic = {}
    buff_dic.update({"type": ws.cell(row_index, 8).value})
    buff_dic.update({"name": clean_null(ws.cell(row_index,9).value)})
