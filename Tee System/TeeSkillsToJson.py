...
while ws.cell(row_index,1).value != None:
    ...
    buff_dic.update({"name": clean_null(ws.cell(row_index,6).value)})
    buff_dic.update({"icon": clean_null(ws.cell(row_index,7).value)})
    buff_dic.update({"desc": clean_null(ws.cell(row_index,8).value)})
    buff_dic.update({"value": ws.cell(row_index, 9).value})
    ...
    for x in range(6):
        if(ws.cell(row_index, 10+2*x).value!= None) and (ws.cell(row_index,10+2*x).value!= ""):
            prize_list.append(ws.cell(row_index,10+2*x).value)
