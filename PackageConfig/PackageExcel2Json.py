...
while ws.cell(row_index, 1).value != None:
    ...
    for index in range(5):
        if ws.cell(row_index, 5+6*index).value !=None:
            ...
            dic_prop.update({"prop_id": ws.cell(row_index,5+6*index).value})
            dic_prop.update({"prop_num": ws.cell(row_index,6+6*index).value})
            dic_prop.update({"prop_type": ws.cell(row_index,7+6*index).value})
            dic_prop.update({"prop_color": ws.cell(row_index,8+6*index).value})
            if ws.cell(row_index, 9+6*index).value != None:
                dic_prop.update({"chest_type": ws.cell(row_index,9+6*index).value})
